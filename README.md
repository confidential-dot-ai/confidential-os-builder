...
Confidential OS Builder (`confos`) is a confidential VM image builder for
AMD SEV-SNP and Intel TDX. It produces bit-identical, attestable disk images
from declarative configuration. The `build` command builds a dm-verity
protected root filesystem, bundles it into a Unified Kernel Image (UKI),
wraps it in an IGVM for measured launch on SNP hardware, and precomputes the
TDX measurement registers for the same artifacts (see [Scope](#scope)).

## Running confos-built VMs

You can use the base images built by `confos` without installing the tool.

```
mkdir confos-base; cd confos-base
oras pull ghcr.io/confidential-dot-ai/confidential-os-builder:base
qemu-system-x86_64 \
  -machine q35 \
  -kernel uki.efi \
...
| [Threat model](docs/THREAT_MODEL.md) | What confos images defend against, trust assumptions, explicit non-goals |
...
bin/setup
bin/confos --help # builds confos using cargo and then runs it
```
...
|----------|---------------------------------------|----------|
| **SEV-SNP** | one entry per `--smp` in `snp_variants[]`: launch digest, IGVM file, page/VMSA counts | confos's IGVM-aware edk2 fork (default `output/OVMF.fd`) |
...
| `-c, --cloud-init <PATH>` | (none) | Standard NoCloud `#cloud-config` user-data, baked into the measured root at `/var/lib/cloud/seed/nocloud/user-data`. That seed is the only datasource: an attached `cidata` disk is ignored, so no unmeasured user-data reaches the guest. At runtime it can write under `/var`, `/home`, `/root`, and `/tmp`, but the immutable root prevents writes to undeclared paths under `/etc` and `/usr`, and the kernel's IPE policy refuses to execute anything outside the verity root, so `runcmd` (a script written under `/var` and executed) does not work; `bootcmd` does, because cloud-init hands it to `/bin/sh`. Bake packages, configuration, units, and scripts with `--package` and `--extra` instead (see the [tutorial](docs/TUTORIAL.md)). |
...
| `--profile <NAME>` | (none) | Enable a shipped or externally staged mkosi profile. Repeatable and composable (`--profile attest --profile ssh`). Shipped profiles: `dev` adds passwordless serial-root autologin and `console=ttyS0` while keeping the production root layout; pair it with `--kernel-config-fragment kernel/dev.config`, and never deploy it because the host controls the serial port. `attest` adds the digest-pinned attestation-api service. `ssh` adds OpenSSH; its host keys regenerate on the unattested `/etc/ssh` state overlay. `gpu` and `attest-gpu` add the NVIDIA stack and GPU-capable attestation. The c8s profile lives in the c8s repository and is staged with `--profile-dir` (see [docs/C8S-IMAGE.md](docs/C8S-IMAGE.md)). Every profile changes the image measurement. |
| `--profile-dir <DIR>` | (none) | Enable an mkosi profile that lives **outside** this repo, so a consumer repo can own its image profile while confos stays the builder. `DIR` must contain an `mkosi.conf`; it is copied to `mkosi/base/mkosi.profiles/<basename>/` for the build's duration and enabled as if passed via `--profile <basename>`, then removed. Repeatable. The basename is load-bearing — sync hooks that self-reference their own profile path (`$SRCDIR/mkosi.profiles/<name>/…`) only resolve if the directory keeps the name they were written for — and it must not collide with a shipped profile. Naming the same basename in `--profile` too pins its position in the config-merge order. The directory must be self-contained: symlinks and relative `Include=` paths in its config tree that leave it are rejected, since staging re-parents both (links under its own `mkosi.extra/` are exempt — those resolve inside the built image). The root is immutable at runtime; if the profile's workload writes anywhere outside `/var`, `/home`, `/root`, `/tmp` (rke2's `/etc/rancher`, say), declare each such directory — which must already exist in the image — one per line in `mkosi.extra/usr/lib/confai/state.d/<NN-name>.conf`, and the initrd gives it an ephemeral writable overlay. Changes the image measurement exactly as an in-tree profile would. |
| `--sync-input <NAME=VALUE>` | (none) | Write `VALUE` (newline-terminated) to `mkosi/base/mkosi.local/.confos-sync-inputs/<NAME>` before the build, for a profile's `mkosi.sync` hook to read. mkosi runs under `sudo` with a stripped environment, so this is the sanctioned way to hand a profile a build-time value (e.g. a component git ref) without hand-editing files in this repo. Repeatable; `NAME` must match `[A-Za-z0-9_-]+`. The files are removed after the build, and a leftover from an interrupted build is cleared by the next one rather than silently reused. |
...
### `confos run` — boot a built VM in QEMU

```bash
confos run [OPTIONS] [DIR]
```
...
| `--firmware <PATH>` | (manifest, or arg) (env: `CONFOS_FIRMWARE`) | OVMF firmware override. Needed when the image was built with `--skip-igvm` and you're booting on KVM (which needs the firmware separately rather than as part of an IGVM). |

`confos run` auto-detects the best available QEMU tier:
...
A booted CVM's root is the read-only verity image; its writable state
directories (`/var`, `/home`, `/root`, `/tmp` — see the state.d note under
`--profile-dir`) are overlays whose upper layer defaults to a 2G RAM tmpfs,
so workloads that need more room run out of space. Attach an **ephemeral
encrypted scratch disk** to expand them:

```bash
confos run output/NAME --scratch 20G
```
...
persisted), formats it, and uses it as the state overlays' upper layer — so
`/var` and the other state directories gain the space transparently.
Undeclared paths—including `/usr` and most of `/etc` in shipped
profiles—remain read-only.

The disk is **ephemeral**: the initrd rekeys and reformats it on every boot.
Under SNP or TDX, guest-memory protection hides the ephemeral key, so the host
sees only ciphertext and cannot recover the contents after shutdown. A
plain-VM run provides neither guarantee because the host can inspect guest
memory. In production, attach your own block device with
`serial=confai-scratch` on the virtio-blk device instead of using `--scratch` (see
[Deploying](docs/DEPLOYING.md#storage) for the QEMU arguments).

Confidential OS Builder currently has no persistent-disk convention: everything in the guest is
...
### `confos kernel` — (re)build the custom kernel

Usually called transparently by `confos build`. Run directly when you've edited a fragment or bumped the kernel version. (It's a maintenance helper, hidden from `confos --help`, but stable enough to document here.)

```bash
confos kernel [OPTIONS]
```

| Arg / flag | Default | Purpose |
|---|---|---|
| `-o, --output <DIR>` | `output/kernel` | Where the resulting `vmlinuz`, `manifest.json`, build cache live. |
| `--kernel-config-fragment <PATH>` | (none) | Extra config fragment merged after required + hardening + confidential. Omitted → confos's baseline kernel. |
| `--kernel-builder-package <PKG>` | (none) | Extra package installed into the kernel-builder tools tree (where the custom kernel is compiled), not the guest image. Repeatable and comma-separated. Use for build-time tools a fragment needs — e.g. `dwarves` (pahole) when the fragment enables `CONFIG_DEBUG_INFO_BTF`. |
...
sudo -E env "PATH=$PATH" ./target/release/confos kernel
 
# 3. Review the snapshot change and commit it alongside the fragment edit.
git diff kernel/config-x86_64.snapshot
```
 
### `confos push` / `confos pull` — OCI registry transfer

Uses `oras` (must be on PATH).

```bash
confos push [OPTIONS] <DIR>
confos pull <IMAGE> [DIR]
```

Push builds the image reference as `<registry>:<tag>`, so `confos push
output/base` pushes `ghcr.io/confidential-dot-ai/confidential-os-builder:base`. Pull takes a
full image reference and lands in `output/<tag>` unless you pass an explicit
directory: `confos pull ghcr.io/confidential-dot-ai/confidential-os-builder:base` fetches the
...
|---|---|---|
| `--registry <URL>` | `ghcr.io/confidential-dot-ai/confidential-os-builder` (env: `CONFOS_OCI_REGISTRY`) | Registry repository — matches where CI publishes `base`. |
...
### `confos igvm` — generate additional IGVM SMP variants

`confos build` already emits one IGVM per `--smp` value. If you later need additional SMP counts for an existing build (each producing a distinct launch digest, since SMP is part of the measurement), `confos igvm` re-renders IGVMs without rebuilding:

```bash
confos igvm output/myimage --smp 1 2 4 8 --firmware output/OVMF.fd
```
...
| `kernel/hardening.config` | Lockdown LSM, IPE execution policy, KASLR, stack protector, attack-surface trims (USB / PCI hotplug / DRM off, etc.) | Always |
...
Each lineage has its own lockfile: the bare baseline writes the committed `kernel/config-x86_64.snapshot`, and a `--kernel-config-fragment` build writes `config-x86_64-<fragment stem>.snapshot` beside the fragment (e.g. `-c8s`), in the caller's own tree where it can be committed and reviewed, so fragment builds never clobber the committed baseline.
...
erofs root filesystem
    ...
    v
roothash (SHA-256)
    |  embedded in kernel cmdline, and sealed into the kernel's
    |  IPE policy (the only root the kernel will execute from)
    v
...
The kernel is sealed to the root it boots: `confos build` runs mkosi once to learn the roothash, appends it to the committed `kernel/ipe-boot-policy`, relinks the cached kernel with that policy, and runs mkosi again with the sealed kernel (the root partition never contains the kernel, so the roothash is the same both times, and the build fails if it is not). At runtime the Integrity Policy Enforcement LSM then refuses to `exec`, `mmap` executable, or load modules and firmware from anything but the initramfs and that one dm-verity volume, and the initrd drops `CAP_MAC_ADMIN` before `switch_root` so no process in the guest can switch enforcement off. Images that must run code fetched at runtime (container hosts) turn IPE off in their kernel fragment with `# CONFIG_SECURITY_IPE is not set`, which builds in one pass and changes the measurement. See [docs/THREAT_MODEL.md](docs/THREAT_MODEL.md).
...
├── vmlinuz          The kernel inside the UKI: the cached kernel relinked with
│                    this image's roothash sealed into its IPE policy
├── ipe-boot-policy  That sealed policy, as compiled into vmlinuz
├── kernel-seal.log  Build log of the relink
...
│                    output dir is self-contained for `confos run` and `confos push`)
...
Confidential OS Builder uses `mkosi` to build base image for Ubuntu 26.04 (Resolute Raccoon).
...
so `confos run` pops a shell. This changes the image measurement and must not be
used for production images — under the SNP threat model the host controls the
serial port.

## License

Confidential OS Builder is licensed under the [GNU Affero General Public License v3.0](LICENSE).
