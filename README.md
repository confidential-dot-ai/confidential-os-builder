...
| `-c, --cloud-init <PATH>` | (none) | Standard NoCloud `#cloud-config` user-data, baked into the measured root at `/var/lib/cloud/seed/nocloud/user-data`. At runtime it can write under `/var`, `/home`, `/root`, and `/tmp`, but the immutable root prevents writes to undeclared paths under `/etc` and `/usr`. Bake packages and configuration with `--package` and `--extra` instead (see the [tutorial](docs/TUTORIAL.md)). |
