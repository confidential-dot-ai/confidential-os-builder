...
| `-c, --cloud-init <PATH>` | (none) | NoCloud `user-data` file baked into the verity root at `/var/lib/cloud/seed/nocloud/user-data`. Measured into the image. Standard cloud-init `#cloud-config` YAML. The root is immutable at runtime, so user-data can write under `/var`, `/home`, `/root` and `/tmp` but not `/etc` or `/usr` — bake packages and config with `--package`/`--extra` instead (see the [tutorial](docs/TUTORIAL.md)). |
...
`/var` and the other state directories gain the space transparently. `/usr`
and `/etc` stay read-only regardless.
