...
- **Changes measurements.** The guest kernel moves from the EOL 6.16.12 line
  ...
  `hash_pointers=always` on the measured command line. The new legacy-xtables
  gate and default-on deprecated tracefs alias/event-probe interfaces are
  pinned off; downstreams that require `iptables-legacy` must opt in through
  their own kernel fragment.
