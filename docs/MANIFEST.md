...
Confidential OS Builder rejects manifests whose `version` differs from the
version it was built with — there is no cross-version compatibility shim. A v2
manifest fails with an explicit "rebuild with the current confos" error rather
than a field-by-field parse error. Unknown fields are also rejected
...
files' names: `confos build` always records `OVMF.fd` for `inputs.firmware`
...
| `memory` | The `--memory` value. Read by `confos run` to size the VM; not part of any measurement (on TDX this is exactly what the RTMR[0]-unpinning design absorbs). | No — runtime default |
...
| `kernel.trusted_aml_patch_sha256` | SHA-256 of the version-specific kernel enforcement patch. Empty for older manifests |
| `kernel.trusted_aml` | Whether the build passed mandatory configuration checks for the built-in DSDT and kernel AML loading restriction. Defaults to `false` for older manifests; not a runtime test result or independent attestation |
...
`confos build`; `confos igvm` can extend or regenerate entries in place.
...
- The schema evolves with confos; a `version` bump means older manifests must
...
- Within a version, the absent-capable fields are: `inputs.kernel`,
  ...
  `trusted_aml_patch_sha256` default to empty strings, and `trusted_aml`
  defaults to `false`.
- Consumers requiring trusted AML must reject missing/false policy
  metadata and approve the rebuilt image's actual measurements. A boolean
