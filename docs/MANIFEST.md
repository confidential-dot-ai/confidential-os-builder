...
| `kernel.ipe_boot_policy_sha256` | Hash of the committed `kernel/ipe-boot-policy` the sealed policy extends (the sealed text itself is written to `output/<name>/ipe-boot-policy`). Empty when the kernel was built without IPE |
