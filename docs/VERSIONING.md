...
Confidential OS Builder uses semantic versioning and is pre-1.0: minor
releases (0.x → 0.y) may break CLI flags, the manifest schema, and build
behavior; patch releases are fixes. The in-repo crates version independently
(`igvm-tools`, `tdx-measure`) since verifiers install them standalone.

The manifest schema carries its own version (`manifest.json`'s `version`
field, currently 3). Confidential OS Builder refuses to read manifests from a
different schema version — regeneration by rebuild is the only migration path,
which guarantees measurements are recomputed rather than translated. Tooling
you build on the manifest should do the same. See
...
|---|---|
| confos version bump | **Assume yes.** Even without a deliberate pipeline change, confos pins the kernel version, base distro snapshot, and image-assembly details; release notes in [CHANGELOG.md](../CHANGELOG.md) flag measurement-affecting changes explicitly |
...
| `confos push`/`pull`, registry, tags | No — transport only |

If you're using Confidential OS Builder to build your images:

- **Pin the confos commit** you build releases with, alongside your image
  config, so you can reproduce byte-identical artifacts later
  ([VERIFYING.md §3](VERIFYING.md#3-build-reproduction-audit)).
- **Treat a confos upgrade like an image change**: rebuild, re-publish the
